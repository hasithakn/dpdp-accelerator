...
public class ComplaintEventServiceImpl implements ComplaintEventService {
    ...
    public ComplaintCommentCreateResponseDTO addComment(String orgId, String complaintId, String actorUserId,
        ...
        if (hasToStatus) {
            // The comment and the status change it carries must land together, in one
            // transaction - otherwise a failure between the two writes could leave a
            // status-changing comment recorded against a complaint whose status never actually
            // moved, or vice versa.
    ...
    public ComplaintStatusUpdateResponseDTO updateStatus(String orgId, String complaintId, String actorUserId,
        ...
        // Status update and its audit event must land together, in one transaction. Both writes
        // are checked and made to fail the whole transaction (not just skip a write) so a partial
        // failure can never leave the status changed with no record of why, or vice versa.
        Connection conn = DatabaseUtils.getDBConnection();
        try {
            if (!complaintDAO.updateStatus(conn, complaintId, orgId, toStatus, now)) {
                throw new ComplaintException(ComplaintErrorCode.INTERNAL_ERROR,
